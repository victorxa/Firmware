...
public:
	...
	bool getParam(const char *key, T &val) const
	{
		...
		if (!_paramDict.get(key, handle)) {
			ROS_WARN("param key not in dict: %s\n", key);
			return false;
		}
		...
		ROS_INFO("param %s get = %f", key, double(val));
		return true;
	}
	...
	void setParam(const char *key, const T &val)
	{
		...
		if (handle == PARAM_INVALID) {
			ROS_WARN("error finding param: %s\n", key);
			// add key if it doesn't exist at runtime
			_paramDict.put(key, handle);
			ROS_INFO("param %s set", key);
		}

		if (handle != PARAM_INVALID) {
			ROS_INFO("settings %s to %f", key, double(val));
			param_set(handle, &val);

		} else {
			ROS_WARN("param %s handle inavlid", key);
		}
	}
